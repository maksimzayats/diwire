...
def test_resolve_raises_when_no_bound_resolver() -> None:
...
@pytest.mark.asyncio
async def test_aresolve_raises_when_no_bound_resolver() -> None:
    context = ProviderContext()
...
def test_enter_scope_raises_when_no_bound_resolver() -> None:
    context = ProviderContext()
...
def test_nested_sync_context_manager_restores_previous_resolver() -> None:
    ...
    with pytest.raises(DIWireProviderNotSetError):
        context.resolve(str)
