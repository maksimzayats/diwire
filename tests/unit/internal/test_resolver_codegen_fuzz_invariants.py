...
def _build_random_container(
    ...
    *,
    ...
    for index in range(node_count):
        ...
        state = (state * 1103515245 + 12345) & 0x7FFFFFFF
        lifetime = (Lifetime.TRANSIENT, Lifetime.SINGLETON, Lifetime.SCOPED)[state % 3]
...
def test_fuzz_codegen_raises_for_cycle_graph() -> None:
    ...
    container = Container()
    container.register_concrete(_CycleA, concrete_type=_CycleA, lifetime=Lifetime.SINGLETON)
    container.register_concrete(_CycleB, concrete_type=_CycleB, lifetime=Lifetime.SINGLETON)
