.. meta::
   :description: Pattern for using diwire with Django: request-bound dependencies and request scopes via @container.inject(scope=Scope.REQUEST).
...
3. Decorate views with ``@container.inject(scope=Scope.REQUEST)`` to open/close a request scope per call.
