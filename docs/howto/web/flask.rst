.. meta::
   :description: Pattern for using diwire with Flask/WSGI: request scopes via @container.inject(scope=Scope.REQUEST) and request-bound dependencies.
...
3. Decorate views with ``@container.inject(scope=Scope.REQUEST)`` (the wrapper opens/closes a request scope per call).
